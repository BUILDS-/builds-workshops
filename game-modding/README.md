...
This "changes to master" model affects how we can think about mods: essentially, most (static) changes to Skyrim are just modifications to the initial game state. The Creation Kit allows you to change this initial game state, providing access to every object in the game along with their properties and relationships to each other. Any changes are stored in an .esp file, short for "Elder Scrolls **Plugin** file, running on top of an .esm (or "Elder Scrolls **Master**" file) where the game is stored.
